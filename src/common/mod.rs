...
pub unsafe fn is_dead(module_accessor: &mut app::BattleObjectModuleAccessor) -> bool {
    ...
    // To prevent this matching the situation after a L + R + A reset, we check the status of the Pokemon during the last command flag cat run.
    if is_ptrainer(module_accessor) {
        ptrainer_switch_dead = 
            ...
            && (save_states::get_lccs(module_accessor) == FIGHTER_STATUS_KIND_STANDBY);
    }
