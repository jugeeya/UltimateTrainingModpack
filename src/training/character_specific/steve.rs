...
unsafe fn save(module_accessor: &mut app::BattleObjectModuleAccessor) -> SteveState {
    let mat_g1 = WorkModule::get_int(
        module_accessor,
        *FIGHTER_PICKEL_INSTANCE_WORK_ID_INT_MATERIAL_NUM_GRADE_1,
    );
    let mat_wood = WorkModule::get_int(
        module_accessor,
        *FIGHTER_PICKEL_INSTANCE_WORK_ID_INT_MATERIAL_NUM_WOOD,
    );
    let mat_stone = WorkModule::get_int(
        module_accessor,
        *FIGHTER_PICKEL_INSTANCE_WORK_ID_INT_MATERIAL_NUM_STONE,
    );
    let mat_iron = WorkModule::get_int(
        module_accessor,
